...
func TestModify(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "TC3", req: newreq("GET", testServer.URL+"/modify/img/TestInput.png?mode=2", nil), status: 200},
		{name: "TC4", req: newreq("GET", testServer.URL+"/modify/img/TestInput.png?mode=2&number=100", nil), status: 200},
	}
	for _, tests := range testCases {
		t.Run(tests.name, func(t *testing.T) {
			resp, err := http.DefaultClient.Do(tests.req)
